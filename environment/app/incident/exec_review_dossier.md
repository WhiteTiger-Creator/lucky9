...
> **Board decision (2026-06-02 - PX-3342)** Moreau: sandbox probe pressure. Each session carries a sandbox probe over the half-open range [end_ms - 180, end_ms + 1), measured separately against the `all`-scoped sandbox windows and against the windows recorded for the session's own class - each scope read on its own, with no `all` fallback. `sandbox_pressure_score = (sbx_all_probe_ms // 26) + ceil(sbx_class_probe_ms / 18) + sandbox_segment_count`. The all half keeps its FLOOR and the class half ROUNDS UP. `sandbox_segment_count` is the NUMBER OF COMPACTED OVERLAP SEGMENTS between the session and the sandbox windows resolved for that session by the ordinary layer rule - the class's own windows, or the `all` scope only when the class has none - i.e. the very same span set whose lengths give `sandbox_overlap_ms`. It is NOT counted over the probe scopes and the two scopes are NOT summed. ROUNDING: sbx_all_probe_ms // 26 = FLOOR. ROUNDING: sbx_class_probe_ms // 18 = CEIL.
> **Board decision (2026-06-02 - PX-3344)** Moreau: audit probe pressure. The audit probe uses the WIDER range [end_ms - 240, end_ms + 1) and its halves round in the OPPOSITE directions to the sandbox probe: `audit_pressure_score = ceil(aud_all_probe_ms / 34) + (aud_class_probe_ms // 22) + audit_segment_count`. No direction here may be inferred from the sandbox family; the all half ROUNDS UP and the class half is FLOORED. `audit_segment_count` is likewise the number of compacted overlap segments between the session and the audit windows resolved by the ordinary layer rule - the same span set behind `audit_overlap_ms` - never a count over the probe scopes and never a sum across scopes. ROUNDING: aud_all_probe_ms // 34 = CEIL. ROUNDING: aud_class_probe_ms // 22 = FLOOR.
> **Board decision (2026-06-04 - PX-3346)** Moreau: containment index and its effect on priority. `containment_index = sandbox_pressure_score + audit_pressure_score + (adjusted_runtime_ms // 40)`, the runtime term FLOORED. A session earns `urgent` when `ledger_runtime_ms >= 300` OR `containment_index >= 12` OR `exec_count >= 3`; the `critical` rule is unchanged. The index is also appended to the burst digest payload after `ledger_runtime_ms`. ROUNDING: adjusted_runtime_ms // 40 = FLOOR.
...
> **Board decision (2026-06-06 - PX-3348)** Moreau: lateral-movement exposure. The host service-dependency graph is read from the fixed absolute path `/app/data/host_dependency_edges.json`, an object mapping each source host to an object of downstream host -> integer edge weight; `--input` never relocates it. For a session on host H, enumerate every simple directed path of 1 to N edges leaving H, where a path's weight is the sum of its edge weights and its node set is the non-origin hosts it visits; H itself is never revisited, so an edge leading back into H is not a path. N is the hop bound for the session's `lead_class`: 3 for `system` and `service`, 2 for `batch` and `adhoc`. `lateral_exposure_score` is the MAXIMUM TOTAL WEIGHT of a set of those paths that pairwise share no non-origin host - a node-disjoint packing, NOT the sum of each reachable host's strongest path. Two paths through a shared intermediate cannot both be counted, so a host reachable only through an already-spent intermediate adds nothing further. An origin with no outgoing edge scores 0.
Okonkwo re-dressed 739 terminations on the atrium riser during the relief shift; cabling only.
Escort-desk staffing for coldroom held at 776 through the night shift with no reader contention.
> **Board decision (2026-06-06 - PX-3350)** Aliyev: lateral exposure reporting. `lateral_reachable_hosts` is the JSON array, sorted ascending, of every host reachable from the session host within the same hop bound PX-3348 sets for that lead class; the origin is never a member. `lateral_path_digest` is the first 12 lowercase hex characters of sha256 over `origin|lateral_exposure_score|` followed by `;`-joined `target:weight:path` triples, one per reachable host in ascending target order, where weight is that target's STRONGEST bounded simple path - the per-target maximum, which is deliberately not how the score itself is computed - and path is that path's hosts joined by `>` with the origin included. Where two equal-weight paths reach the same target, the one whose `>`-joined rendering sorts lowest wins. All three fields ride on every queue row. `lateral_exposure_score` is appended to the burst digest payload after `containment_index`, and a session also earns `urgent` when `lateral_exposure_score >= 16`. Each host's `max_lateral_exposure_score` joins the host matrix, and the summary carries the fleet maximum under the same key.

> **Board decision (2026-06-08 - PX-3352)** Aliyev: lateral contention (final). Each session's `lateral_exposure_score` is packed independently under PX-3348, so a host reachable from several origin hosts is currently credited to every one of them. A target host reachable from MORE THAN ONE origin host is charged to exactly ONE of them. For each such contended target, take the origin hosts that reach it and pick the OWNER: the claimant whose strongest bounded simple-path score TO THAT TARGET is highest, ties broken by the lexicographically smallest origin host name. This is the per-target path score — the greatest summed edge weight of any simple path to that target — and it is deliberately NOT the origin's overall packed `lateral_exposure_score`; the two pick different owners. For this attribution, reachability and the per-target scores use the FULL hop bound of 3 regardless of any session's lead_class, so ownership is a property of the host, not of the session being scored. The owner adds that same per-target score to the `lateral_exposure_score` of EVERY session on the owner host; every other claimant adds nothing, and a target reachable from exactly one origin host is not contended and adds nothing. Origins are the hosts that carry sessions. The bonus is folded in BEFORE `lateral_path_digest` is taken, so the digest and the `lateral_exposure_score >= 16` urgent rule and the burst digest all commit to the attributed score. `lateral_reachable_hosts` and the per-target `target:weight:path` triples are unaffected — attribution changes what a host SCORES, never what it reaches. Worked example: host t is reached by origins a (per-target path score 9) and b (per-target path score 4); a owns t and every session on a adds 9, b adds 0, and t still appears in both hosts' `lateral_reachable_hosts`.

> **Board decision (2026-06-08 - PX-3354)** Moreau: containment-index rounding, final. The floored runtime term in the containment index was shedding a partial step, so it now ROUNDS UP: `containment_index = sandbox_pressure_score + audit_pressure_score + ceil(adjusted_runtime_ms / 40)`. In integer arithmetic ceil(x/40) is -(-x // 40). This revises the floored `adjusted_runtime_ms // 40` recorded in PX-3346 on that one rounding point only; the index composition, its place in the burst digest payload, and the `containment_index >= 12` urgent rule are all unchanged. ROUNDING: adjusted_runtime_ms // 40 = CEIL.
Haugen archived 813 controller diagnostics from eastwing; retention only, no bearing on the rollup.
