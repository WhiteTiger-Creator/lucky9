...
> **Board decision (2026-06-02 - PX-3342)** Moreau: sandbox probe pressure. Each session carries a sandbox probe over the half-open range [end_ms - 180, end_ms + 1), measured separately against the `all`-scoped sandbox windows and against the windows recorded for the session's own class - each scope read on its own, with no `all` fallback. `sandbox_pressure_score = (sbx_all_probe_ms // 26) + ceil(sbx_class_probe_ms / 18) + sandbox_segment_count`. The all half keeps its FLOOR and the class half ROUNDS UP. ROUNDING: sbx_all_probe_ms // 26 = FLOOR. ROUNDING: sbx_class_probe_ms // 18 = CEIL.
> **Board decision (2026-06-02 - PX-3344)** Moreau: audit probe pressure. The audit probe uses the WIDER range [end_ms - 240, end_ms + 1) and its halves round in the OPPOSITE directions to the sandbox probe: `audit_pressure_score = ceil(aud_all_probe_ms / 34) + (aud_class_probe_ms // 22) + audit_segment_count`. No direction here may be inferred from the sandbox family; the all half ROUNDS UP and the class half is FLOORED. ROUNDING: aud_all_probe_ms // 34 = CEIL. ROUNDING: aud_class_probe_ms // 22 = FLOOR.
